...
#include "lab08.public.cuh"
#include "lab08cpu.public.hpp"
...
TEST (MPIInputTest, t1)
{
    ...
    EXPECT_EQ(
        solver.boundaries,
        Lab08::Boundaries({7.0, 7.0, 7.0, 7.0, 7.0, 7.0})
    )
    << "Wrong boundaries\n" << "Rank: " << solver.rank;
...
TEST (MPIInputTest, t2)
{
    ...
    EXPECT_EQ(
        solver.boundaries,
        Lab08::Boundaries({7.0, 0.0, 5.0, 0.0, 3.0, 0.0})
    )
    << "Wrong boundaries\n" << "Rank: " << solver.rank;
...
TEST (MPIOutputTest, t1)
{
    ...
    int cmp_result = system("python cmp.py t1.out mpi.out");
...
TEST (MPIOutputTest, t2)
{
    ...
    int cmp_result = system("python cmp.py t2.out mpi.out");

    EXPECT_EQ(cmp_result, 0);
}

TEST (MPIOutputTest, final0)
{
    Lab08 solver(argc_, argv_);
    ...
    int cmp_result = system("python cmp.py final.out mpi.out");
...
TEST (CpuInputTest, t1)
{
    ...
    EXPECT_EQ(
        solver.boundaries,
        Lab08cpu::Boundaries({7.0, 7.0, 7.0, 7.0, 7.0, 7.0})
    )
    << "Wrong boundaries";
...
TEST (CpuInputTest, t2)
{
    ...
    EXPECT_EQ(
        solver.boundaries,
        Lab08cpu::Boundaries({7.0, 0.0, 5.0, 0.0, 3.0, 0.0})
    )
    << "Wrong boundaries";
...
TEST (FinalTest, single)
{
    ...
    if (get_rank() == 0)
    {
        ...
        Lab08cpu cpu_solver(argc_, argv_);
